...
export const TodayDashboard: React.FC<TodayDashboardProps> = ({
  ...
  const hiddenIdSet = useMemo(() => {
    ...
    classes.forEach(c => {
      if (c.isHiddenFromHome === true || String(c.isHiddenFromHome) === 'true') set.add(c.id);
    });
    return set;
  ...
  const visibleClasses = useMemo(() => {
    return classes.filter(c => {
      const isExplicitlyHidden = c.isHiddenFromHome === true || String(c.isHiddenFromHome) === 'true';
      const isInHiddenSet = hiddenIdSet.has(c.id);
      return !isExplicitlyHidden && !isInHiddenSet;
    });
  }, [classes, hiddenIdSet]);
