 ...
 * 4. Never commit keys to GitHub (.env is ignored).
...
export function getSupabase(): SupabaseClient | null {
  ...
  if (!url || !key) {
    if (!hasLoggedConfigStatus) {
      console.log('[Supabase DB] Notice: SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY not set in environment. Using local fallback.');
  ...
  if (!supabaseClient) {
    try {
      ...
      if (!hasLoggedConfigStatus) {
        ...
        console.log(`[Supabase DB] Initialized server-side Supabase client with ${keyType} at ${url}`);
        hasLoggedConfigStatus = true;
      }
    } catch (err) {
      console.error('[Supabase DB] Error initializing server-side Supabase client:', err);
      return null;
    }
  }
